...
def generate_point_clouds(
    batch_size: int, n_points: int, m_points: int, device: str = "cuda"
...
def theoretical_flops_emd(n_points: int, m_points: int) -> int:
    ...
    distance_matrix_flops = (
        n_points * m_points * 3 * 2
    )  # multiply + subtract for each dimension
    matching_flops = (
        min(n_points, m_points) ** 3
    )  # Approximation for Hungarian algorithm
...
def theoretical_flops_chamfer(n_points: int, m_points: int) -> int:
    ...
    distance_matrix_flops = (
        n_points * m_points * 3 * 4
    )  # (x1-x2)^2 + (y1-y2)^2 + (z1-z2)^2 for each pair
...
def main():
    ...
    for batch_size, n_points, m_points in configs:
        ...
        try:
            chamfer_results = benchmark_operation(
                chamfer_distance, p1, p2, "Chamfer Distance"
            )
    ...
    print(f"{'='*80}")
    print(
        f"{'Configuration':<20} {'Operation':<15} {'Mean Time (ms)':<15} {'GFLOPS':<10}"
    )
    print("-" * 80)
