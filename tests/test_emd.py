...
def reference_emd(p1, p2):
    ...
    for i in range(batch_size):
        ...
        emd_dist[i] = cost_matrix[row_ind, col_ind].sum() / min(num_points1, num_points2)
...
class TestEMDDistance(TestCase):
    ...
    def sample_inputs(self, device, batch_size=1, n_points=64, m_points=128, requires_grad=False, dtype=torch.float64):
        p1 = torch.randn(batch_size, n_points, 3, device=device, requires_grad=requires_grad, dtype=dtype)
        p2 = torch.randn(batch_size, m_points, 3, device=device, requires_grad=requires_grad, dtype=dtype)
        return p1, p2
    ...
    def test_static_values(self):
        device = "cuda"
        p1 = torch.tensor([[[1.7, -0.1, 0.1], [0.1, 1.2, 0.3]]], dtype=torch.float32, device=device)
        p1 = p1.repeat(3, 1, 1)
        p2 = torch.tensor([[[0.3, 1.8, 0.2], [1.2, -0.2, 0.3]]], dtype=torch.float32, device=device)
        p2 = p2.repeat(3, 1, 1)
        ...
        dist_expected = torch.tensor([0.355, 0.355, 0.355], device=device, dtype=torch.float32)
        self.assertTrue(torch.allclose(dist, dist_expected, atol=1e-3))

        p1_grad_expected = torch.tensor([[[ 0.2500,  0.0500, -0.1000],
                                          [-0.1000, -0.3000,  0.0500]],
                                         [[ 1.0000,  0.2000, -0.4000],
                                          [-0.4000, -1.2000,  0.2000]],
                                         [[ 0.1667,  0.0333, -0.0667],
                                          [-0.0667, -0.2000,  0.0333]]], device=device, dtype=torch.float32)
        
        p2_grad_expected = torch.tensor([[[ 0.1000,  0.3000, -0.0500],
                                          [-0.2500, -0.0500,  0.1000]],
                                         [[ 0.4000,  1.2000, -0.2000],
                                          [-1.0000, -0.2000,  0.4000]],
                                         [[ 0.0667,  0.2000, -0.0333],
                                          [-0.1667, -0.0333,  0.0667]]], device=device, dtype=torch.float32)
